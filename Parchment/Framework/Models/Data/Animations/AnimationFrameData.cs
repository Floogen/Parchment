...
namespace Parchment.Framework.Models.Data.Animations
{
    ...
    {
        /// <summary>The source point for this frame, understood by every element that draws a sprite and rejected at load on the ones that draw only text.
        /// It moves where in the sheet the art is read from and never how much of it, so a nine-sliced element keeps the border and the inset it was measured with.
        /// Automatically matches the dimensions of the element's <see cref="ISprite.TextureSourceRectangle"/>, or of the item's sprite when <see cref="ImageElementData.ItemId"/> is used.
        ...
        /// <summary>How long this frame is shown, in milliseconds. When null, the element's <see cref="ElementData.FrameDuration"/> is used.</summary>
        ...
        /// Only used by an Image, the one element drawn as a single quad with nothing inside it measured against its size, and rejected at load elsewhere. See <see cref="ElementData.SupportsFrameScale"/>.
        ...
        /// Understood by every element type, and the one field that makes a frame worth putting on something that isn't a sprite. A container carries its children and its own layers along with it.
