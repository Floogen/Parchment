...
| `ParseTokenizableStrings` <span class="opt">optional</span> | `bool` | `true` | Whether the game's `[Token]` [tokenizable strings](../concepts/actions.md#game-tokens) are resolved in this entry's `Condition` and actions. A keybind has no element to take this from, so it carries its own. |
...
!!! note "A bind has no element to read from"
    A keybind belongs to the page or to the book rather than to an element, so `%Input%` on its own, `%Item%` and `%Tags%` have nothing to answer with and are left in place. `%Input:someId%`, `%Variable:someId%` and the grid tokens work here as they do anywhere, as do the game's own `[Token]` forms.
